...
[TestFixture]
public class SimpleStagePipelineTests
{
    private const string YamlPath = "pipelines/SimplePipeline/simple_stage_pipeline.yaml";
    ...
    [Test]
    public void VerifyStagesAndTasks()
    {
        var result = new PipelineTester().WithPipeline(YamlPath).Run();

        result.ShouldHaveStage("Build Stage");
        result.ShouldHaveJob("Build Stage", "Build .NET Project");
        result.ShouldHaveTask("UseDotNet@2");
        result.ShouldHaveTask("DotNetCoreCLI@2");
        result.ShouldHaveStep("Use .NET 8.0");
        result.ShouldHaveStep("Restore dependencies");
        result.ShouldHaveStep("Build");
        result.ShouldHaveStep("Test");
    }
}
